...
final class WP_Recovery_Mode_Key_Service {
	 ...
	 *
	 * @since 5.2.0
	 * @since x.y.z The stored key is now hashed using sha1 via wp_hash() instead of phpass.
	 *
	...
	public function validate_recovery_mode_key( $token, $key, $ttl ) {
		...
		// @TODO check this function is always available when this is called
		if ( ! hash_equals( $record['hashed_key'], wp_hash( $key, 'auth', 'sha1' ) ) ) {
	 ...
	 *
	 * @since 5.2.0
	 * @since x.y.z Each key is now hashed using sha1 via wp_hash() instead of phpass.
	 ...
	 *
	 * @since 5.2.0
	 * @since x.y.z Each key should now be hashed using sha1 via wp_hash() instead of phpass.
	 *
	 * @param array $keys {
	 *     Associative array of token => data pairs, where the data is an associative
	 *     array of information about the key.
